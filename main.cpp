...
int main ()
{
  ...
  **/
  ...
  //Initialize pid_steer and pid_throttle
  //pid_steer.Init(0.25,0.05,0.5,-1.2,1.2);// fast to turn right crash the wall
  pid_steer.Init(0.1,0.05,0.5,-1.2,1.2);
  pid_throttle.Init(0.1,0.05,0.1,-1.0,1.0);
...
//           // Update the delta time with the previous command
          ...
          **/
          // Look for the closed waypoints
          int closed_index=0;
          double closed_dis=pow((x_position - x_points[closed_index]),2) + pow((y_position - y_points[closed_index]),2);
          for (int i =0; i< x_points.size(); ++i)
          {
            double dis = pow((x_position - x_points[i]),2) + pow((y_position - y_points[i]),2);
            if (dis < closed_dis)
            {
              closed_dis = dis;
              closed_index = i;
         ...
         //error_steer= desired position - actual position  
          double desired_steer=angle_between_points(x_position,y_position,x_points[closed_index],y_points[closed_index]);
          error_steer = correct_angle(desired_steer - yaw); 
...
//           // Update the delta time with the previous command
          ...
          **/
          ...
          error_throttle = velocity - v_points[closed_index];
